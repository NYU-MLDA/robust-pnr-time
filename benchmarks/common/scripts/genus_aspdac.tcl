...
set paths [report_timing -from [all_registers] -to [all_registers] -logic_levels 10000 -logic_levels_tcl_list -max_paths 10000]

set fl [open syn_gen_reg2reg_paths.csv w]
foreach path $paths {puts $fl "[lindex $path 0], [lindex $path 1], [lindex $path 2]"}
report_timing -from [all_registers] -to [all_registers] -max_paths 10000 > ${RESULTS_DIR}/${TOP_DESIGN}_reg2reg_generic_timing.rpt
close $fl


set paths [report_timing -from [all_inputs] -to [all_registers] -logic_levels 10000 -logic_levels_tcl_list -max_paths 10000]

set fl [open syn_gen_in2reg_paths.csv w]
foreach path $paths {puts $fl "[lindex $path 0], [lindex $path 1], [lindex $path 2]"}
report_timing -from [all_inputs] -to [all_registers] -max_paths 10000 > ${RESULTS_DIR}/${TOP_DESIGN}_in2reg_generic_timing.rpt
close $fl

set paths [report_timing -from [all_registers] -to [all_outputs] -logic_levels 10000 -logic_levels_tcl_list -max_paths 10000]

set fl [open syn_gen_reg2out_paths.csv w]
foreach path $paths {puts $fl "[lindex $path 0], [lindex $path 1], [lindex $path 2]"}
report_timing -from [all_registers] -to [all_outputs] -max_paths 10000 > ${RESULTS_DIR}/${TOP_DESIGN}_reg2out_generic_timing.rpt
close $fl

set paths [report_timing -from [all_inputs] -to [all_outputs] -logic_levels 10000 -logic_levels_tcl_list -max_paths 10000]

set fl [open syn_gen_in2out_paths.csv w]
foreach path $paths {puts $fl "[lindex $path 0], [lindex $path 1], [lindex $path 2]"}
report_timing -from [all_inputs] -to [all_outputs] -max_paths 10000 > ${RESULTS_DIR}/${TOP_DESIGN}_in2out_generic_timing.rpt
close $fl
...
set fl [open syn_gen_reg2reg_paths.csv w]
foreach path $paths {puts $fl "[lindex $path 0], [lindex $path 1], [lindex $path 2]"}
report_timing -from [all_registers] -to [all_registers] -max_paths 10000 > ${RESULTS_DIR}/${TOP_DESIGN}_reg2reg_techmap_timing.rpt
close $fl
...
set fl [open syn_gen_in2reg_paths.csv w]
foreach path $paths {puts $fl "[lindex $path 0], [lindex $path 1], [lindex $path 2]"}
report_timing -from [all_inputs] -to [all_registers] -max_paths 10000 > ${RESULTS_DIR}/${TOP_DESIGN}_in2reg_techmap_timing.rpt
close $fl
...
set fl [open syn_gen_reg2out_paths.csv w]
foreach path $paths {puts $fl "[lindex $path 0], [lindex $path 1], [lindex $path 2]"}
report_timing -from [all_registers] -to [all_outputs] -max_paths 10000 > ${RESULTS_DIR}/${TOP_DESIGN}_reg2out_techmap_timing.rpt
close $fl
...
set fl [open syn_gen_in2out_paths.csv w]
foreach path $paths {puts $fl "[lindex $path 0], [lindex $path 1], [lindex $path 2]"}
report_timing -from [all_inputs] -to [all_outputs] -max_paths 10000 > ${RESULTS_DIR}/${TOP_DESIGN}_in2out_techmap_timing.rpt
close $fl
